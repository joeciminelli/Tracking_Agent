...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = [dist for dist in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    variableDomainsDict[OBS1] = [dist for dist in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:
            currentFactors, joinedFactor = joinFactorsByVariable(currentFactors, eliminationVariable)

            if len(joinedFactor.unconditionedVariables()) != 1:
                eliminatedFactor = eliminate(joinedFactor, eliminationVariable)
                currentFactors.append(eliminatedFactor)
        
        normalized_and_joined = normalize(joinFactors(currentFactors))
        
        return normalized_and_joined
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        "*** YOUR CODE HERE ***"
        if self.total() > 0:
            normalizer = 1.0/self.total()
            for i in self:
                self[i] = self[i]*normalizer
    ...
    def sample(self):
        ...
        bigList = []
        for item in self:
            for amount in range(self.get(item)):
                bigList.append(item)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        for coord in self.beliefs:
            self.beliefs[coord] = self.beliefs[coord] * self.getObservationProb(observation, gameState.getPacmanPosition(), 
            coord, self.getJailPosition())
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for position in allPositions:
            finalDistributions[position] = self.getPositionDistribution(gameState, position)
        original_beliefs = self.beliefs.copy()
        for finalPosition in allPositions:
            probability = 0.0
            for initialPosition in allPositions:
                probability += self.beliefs[initialPosition] * finalDistributions[initialPosition][finalPosition]
            original_beliefs[finalPosition] = probability
        self.beliefs = original_beliefs
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        while numParticles != 0:
            ...
            if numParticles > numPositions:
                self.particles = self.particles + self.legalPositions
                numParticles = numParticles - numPositions
            else:
                self.particles = self.particles + self.legalPositions[0:numParticles]
                numParticles = 0
    ...
    def getBeliefDistribution(self):
        ...
        if distribution.total() > 0:
            normalizer = 1.0/distribution.total()
            for i in distribution:
                distribution[i] = distribution[i]*normalizer
