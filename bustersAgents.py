...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        #initialize big distance and blank position 
        minDistance = 1000000
        ghostPosition = ''
        
        #get nearest ghost position
        for position in livingGhostPositionDistributions:
            currentPosition = position.argMax()        
            currentDistance = self.distancer.getDistance(pacmanPosition, currentPosition)
            if currentDistance < minDistance:
                minDistance = currentDistance
                ghostPosition = currentPosition
        
        #initialize big distance and blank action
        minDistance = 1000000
        bestAction = ''
        
        #get best action based on nearest ghost position
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            currentDistance = self.distancer.getDistance(successorPosition, ghostPosition)
            if currentDistance < minDistance:
                minDistance = currentDistance
                bestAction = action
